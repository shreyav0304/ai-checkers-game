...
class Board:
    ...
    def create_board(self):
        self.board_state = [[EMPTY for _ in range(COLS)] for _ in range(ROWS)]
        for r in range(ROWS):
            for c in range(COLS):
                if (r + c) % 2 != 0:
                    if r < 3: self.board_state[r][c] = BLACK
                    elif r > 4: self.board_state[r][c] = RED
        self.red_left = self.black_left = 12
        self.red_kings = self.black_kings = 0
    ...
    def _get_jumps(self, pos):
        ...
        p = self.get_piece(r, c)
        jumps = {}
        move_dirs = [(-1, -1), (-1, 1), (1, -1), (1, 1)] if p in (KING_R, KING_B) else \
                    [(-1, -1), (-1, 1)] if p == RED else [(1, -1), (1, 1)]
    ...
    def evaluate(self):
        return (self.black_left - self.red_left) + (self.black_kings * 1.5 - self.red_kings * 1.5)
        ...
        if maximizing_player:
            ...
            for piece_pos, moves in board.get_all_valid_moves(player_color).items():
                for move_pos in moves.keys():
                    ...
                    temp_board.move(piece_pos, move_pos)
                    evaluation = self.minimax(temp_board, depth-1, alpha, beta, False)[0]
                    if evaluation > max_eval: max_eval, best_board = evaluation, temp_board
            ...
            for piece_pos, moves in board.get_all_valid_moves(player_color).items():
                for move_pos in moves.keys():
                    ...
                    if evaluation < min_eval: min_eval, best_board = evaluation, temp_board
...
class Game:
    ...
    def init_fonts_and_ui(self):
        self.font_sm = pygame.font.SysFont("inter", 18)
        self.font_md = pygame.font.SysFont("inter", 22, bold=True)
        self.font_lg = pygame.font.SysFont("inter", 32, bold=True)
        self.font_title = pygame.font.SysFont("inter", 60, bold=True)
        self.init_menu_buttons()
        self.init_game_buttons()

    def init_menu_buttons(self):
        self.menu_buttons = [
            Button(pygame.Rect(WIDTH//2 - 150, HEIGHT//2 - 60, 300, 60), "Player vs AI", COLOR_GREEN, tuple(min(255,c+20) for c in COLOR_GREEN), self.font_md, lambda: self.start_game("ai")),
            Button(pygame.Rect(WIDTH//2 - 150, HEIGHT//2 + 20, 300, 60), "2 Players", COLOR_BLUE, tuple(min(255,c+20) for c in COLOR_BLUE), self.font_md, lambda: self.start_game("2p")),
            Button(pygame.Rect(WIDTH//2 - 150, HEIGHT//2 + 100, 300, 40), f"Difficulty: {'Easy' if self.difficulty == 2 else 'Medium' if self.difficulty == 4 else 'Hard'}", COLOR_GHOST, COLOR_BORDER, self.font_sm, self.toggle_difficulty)
        ]

    def init_game_buttons(self):
        sidebar_x = BOARD_POS[0] + BOARD_SIZE + PADDING
        self.game_buttons = [
            Button(pygame.Rect(sidebar_x + PADDING, BOARD_POS[1] + 160, 140, 50), "New Game", COLOR_GREEN, tuple(min(255, c+20) for c in COLOR_GREEN), self.font_md, self.reset_to_menu),
    ...
    def reset(self):
        ...
        self.turn = RED
        self.selected_piece, self.valid_moves, self.winner = None, {}, None
        self.history = []
    ...
    def run_menu(self):
        ...
        self.draw_text("AI Checkers", self.font_title, COLOR_TEXT, (WIDTH//2, HEIGHT//2 - 160))
        for button in self.menu_buttons: button.draw(self.screen, pygame.mouse.get_pos())
        pygame.display.update()

    def run_game(self):
        if self.game_mode == "ai" and self.turn == BLACK and not self.winner:
            pygame.time.wait(300)
            red_pieces_before = self.board.red_left
            _, best_board = self.board.minimax(self.board, self.difficulty, -inf, inf, True)
            if best_board:
                self.history.append({'board': self.board.clone(), 'turn': self.turn})
                self.board = best_board
                if self.board.red_left < red_pieces_before:
    ...
    def run_game_over(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT: pygame.quit(), sys.exit()
            if event.type == pygame.MOUSEBUTTONDOWN: self.game_state = "MENU"
        
        self.draw_game_ui()
        overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA); overlay.fill((0,0,0,180))
        self.screen.blit(overlay, (0,0))
        winner_text = "Red Wins!" if self.winner == RED else "Black Wins!"
        self.draw_text(winner_text, self.font_title, COLOR_TEXT, (WIDTH//2, HEIGHT//2 - 40))
        self.draw_text("Click anywhere to return to Menu", self.font_md, COLOR_LABEL, (WIDTH//2, HEIGHT//2 + 40))
        pygame.display.update()
    ...
    def handle_board_click(self, pos):
        ...
        if self.selected_piece:
            if (row, col) in self.valid_moves:
                self.history.append({'board': self.board.clone(), 'turn': self.turn})
                is_capture = self.board.move(self.selected_piece, (row, col))
            ...
            if (row, col) in all_moves:
                if has_jumps and not any(all_moves[(row, col)].values()):
                    return
                
                self.sounds.play("select")
                self.selected_piece = (row, col)
                self.valid_moves = all_moves.get((row, col), {})
    ...
    def undo_move(self):
        ...
        if self.game_mode == 'ai' and self.turn == RED:
        ...
        self.winner, self.selected_piece, self.valid_moves = None, None, {}
        print("Move undone.")
        ...
        for r in range(ROWS):
            for c in range(COLS):
                ...
                if p_val != EMPTY:
                    ...
                    pygame.draw.circle(piece_surf, (0,0,0,50), (SQUARE_SIZE//2, SQUARE_SIZE//2 + 4), radius)
                    pygame.draw.circle(piece_surf, color, (SQUARE_SIZE//2, SQUARE_SIZE//2), radius)
                    
                    if p_val in (KING_R, KING_B):
                        crown_font = pygame.font.SysFont("arial", 30, bold=True)
                        crown = crown_font.render('👑', True, COLOR_KING)
                        piece_surf.blit(crown, (SQUARE_SIZE//2 - crown.get_width()//2, SQUARE_SIZE//2 - crown.get_height()//2))
                    
                    self.screen.blit(piece_surf, (BOARD_POS[0] + c*SQUARE_SIZE, BOARD_POS[1] + r*SQUARE_SIZE))
    ...
    def draw_sidebar(self):
        ...
        card2_rect = pygame.Rect(sidebar_x, card1_rect.bottom + 20, SIDEBAR_WIDTH, 250)
        pygame.draw.rect(self.screen, COLOR_CARD, card2_rect, border_radius=16)
        self.draw_text("This project is for educational purposes only.", self.font_sm, COLOR_LABEL, (card2_rect.centerx, card2_rect.bottom - 40))
    ...
    def get_status_info(self):
        ...
        if self.game_mode == "ai":
            return ("Your Turn (Red)", COLOR_RED_PIECE) if self.turn == RED else ("AI's Turn (Black)", COLOR_BLACK_PIECE)
        else:
            return ("Player 1's Turn (Red)", COLOR_RED_PIECE) if self.turn == RED else ("Player 2's Turn (Black)", COLOR_BLACK_PIECE)
    ...
    def draw_instructions(self):
        ...
        instructions = ["- Select your piece to see its valid moves.", "- If a jump is available, you must take it.", "- Pieces become Kings (👑) at the opponent's back rank.", "- Kings can move and capture forwards and backwards.", "- Use the sidebar to start a new game."]
